...
@bot.on_callback_query(filters.regex(pattern="^(generative_fill_image=.*)$"))
async def generative_fill_image(c:bot, cbq:CallbackQuery):
    ...
    url = api_cli.generative_fill(image_id)
    await c.send_photo(
        chat_id=user_id,
        caption=f"Here's your generative filled image.",
