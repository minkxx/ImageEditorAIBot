...
@bot.on_callback_query(filters.regex(pattern="^(generative_restore=.*)$"))
async def generative_restore(c:bot, cbq:CallbackQuery):
    ...
    url = api_cli.generative_restore(image_id)
    await c.send_photo(
        chat_id=user_id,
        caption=f"Here's your restored image",
