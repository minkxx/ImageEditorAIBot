...
@bot.on_callback_query(filters.regex(pattern="^(optimize_image=.*)$"))
async def optimize_image(c: bot, cbq: CallbackQuery):
    ...
    if image_id in photo_id:
        ...
        url = api_cli.optimize_image(image_id)
        await c.send_photo(
            chat_id=user_id,
            caption=f"Here's your optimized image.",
