...
@bot.on_callback_query(filters.regex(pattern="^(upscale_image=.*)$"))
async def upscale_image(c:bot, cbq:CallbackQuery):
    ...
    url = api_cli.upscale_image(image_id)
    await c.send_photo(
        chat_id=user_id,
        caption=f"Here's your upscaled image",
