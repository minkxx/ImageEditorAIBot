...
def get_latest_local_commit():
    result = subprocess.run(["git", "rev-parse", "HEAD"], cwd=LOCAL_REPO_PATH, capture_output=True, text=True)
    return result.stdout.strip() if result.returncode == 0 else None

async def restart(c, m):
    x = await c.send_message(chat_id=m.chat.id, text="Starting repository tracking...", reply_to_message_id=m.id)
